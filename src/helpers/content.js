const content = {
  title: 'Scott Sullivan UI Engineer',
  description: 'Scott Sullivan UI Engineer',
  canonical: 'https://ssullivan.info/',
  mainHeadingText: 'Scott Sullivan',
  subHeadingText: 'UI Engineer - London / Chicago',
  bioHeading: 'Hello and welcome',
  bioText:
    "I'm a software engineer with 8+ years experience, who is meticulous about quality, meeting requirements within timelines and pushing technical boundaries to deliver outstanding user experiences.",
  bioImg:
    'https://scontent-lhr3-1.cdninstagram.com/vp/32a3801cf287c36ce0d8b09cc0551a61/5DE61703/t51.2885-19/51555128_153971348831636_6628374679469948928_n.jpg?_nc_ht=scontent-lhr3-1.cdninstagram.com',
  bioFavorites: '',
  skills: [
    {
      title: 'Development',
      items: [
        'React | Redux',
        'CSS, SCSS, Styled-Components',
        'Web Performance | PWA',
        'Webpack & Bundle-size Management',
        'Jest Unit Tests w/ Enyme',
        'Cypress E2E & Nightwatch',
      ],
    },
    {
      title: 'Architecture',
      items: [
        'Storybooks Component Libraries',
        'Style-guide Patterns',
        'Headless CMS Integration',
        'Typescript',
        'Boilerplate & Component Scaffolding',
        'Git | Gitflow | Peer Review',
      ],
    },
    {
      title: 'Infrastructure',
      items: [
        'Node.js, Express.js & Hapi.js',
        'SSR Applications',
        'Jenkins CI',
        'Docker Containerization ',
        'Global Package Management',
        'Monorepo & SemVer Practice',
      ],
    },
  ],
  social: {
    linkedIn: 'https://www.linkedin.com/in/scott-j-sullivan-14094a58/',
    github: 'https://github.com/ssulivan11',
    codePen: 'https://codepen.io/ssulivan11',
  },
}

module.exports = content
